...
function createSandboxedBashOps(opts?: {
	...
	onAlways?: (absPath: string) => Promise<void>;
}): BashOperations {
	return {
				...
				child.on("close", async (code) => {
					...
					if (/operation not permitted|EPERM|EACCES/i.test(outputTail)) {
						...
						if (opts?.ctx?.hasUI && opts.ctx.ui?.select && opts.onAlways) {
							hint += `   → You'll get a prompt to allow this path for this cwd.\n`;
						}
					...
					// Ask-tier prompt: AFTER resolve so we don't block the tool reply.
					// Live-reloads the sandbox so the next bash call sees the override.
					if (offending && opts?.ctx?.hasUI && opts.ctx.ui?.select && opts.onAlways) {
						...
						const title = `Layer 1 (bash sandbox) blocked write to:\n  ${absPath}\n\nAllow future bash commands to write here for this cwd?`;
						const options = [
							"always for this cwd — write .pi/sandbox.json + reload sandbox",
						...
						try {
							...
							if (chosen === options[0]) {
								try {
									await opts.onAlways(absPath);
									appendFileSync(auditPath, `${JSON.stringify({ ts, layer: 1, tool: "bash", subject: absPath, decision: "always", cwd: opts.ctx.cwd })}\n`);
									opts.ctx.ui?.notify?.(`pi-sandbox: allowed ${absPath} for ${opts.ctx.cwd} — retry the bash command`, "warning");
								} catch (e) {
									appendFileSync(auditPath, `${JSON.stringify({ ts, layer: 1, tool: "bash", subject: absPath, decision: "always", cwd: opts.ctx.cwd, error: String(e) })}\n`);
...
export default function (pi: ExtensionAPI) {
	...
	/** Persist an "always for this cwd" Layer 1 override and live-reload SandboxManager. */
	async function persistAndReload(absPath: string): Promise<void> {
		const dir = join(localCwd, ".pi");
		const path = join(dir, "sandbox.json");
		let existing: { overrides?: { allowWrite?: string[] } } = {};
		if (existsSync(path)) {
			try {
